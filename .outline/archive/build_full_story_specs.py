# -*- coding: utf-8 -*-
"""
144-Chapter Master Dataset Compiler for Tianque Jinglong Master Outline
Generates vol1_specs.py through vol6_specs.py with zero template reuse.
"""
...
def make_vol_header(vol_num, vol_name):
    return f'''# -*- coding: utf-8 -*-
"""Volume {vol_num}: {vol_name} Specifications (Bespoke Handcrafted)"""

def get_vol{vol_num}_specs(make_beat, make_scene, make_cluster):
    records = []
...
def format_ch_code(n, vol, title, actors, function, delta_st, conf_c, stakes_c, hook_c, beats, scenes, clusters):
    code = []
    code.append(f"    # {n}")
